...
class Page extends State<Accueil> {
  void connexionFireBase() {
    // Fonction de connexion a son compte
    final firebaseAuth =
        FirebaseAuth.instance; //Initialisation de l'instance FireBase
    firebaseAuth
        .signInWithEmailAndPassword(
            // Fonction de FireBase qui permet de s'autenfier qui prend en parametre le mail et le mdp saisie dans le formulaire
            email: mailController.text,
            password: mdpController.text)
        .then((result) {
      bool emailVerified = result.user!
          .emailVerified; // Ensuite on verifie que le compte a bien été verifie si oui on emmene a la page d'Accueil
      if (emailVerified) {
        Navigator.of(context).pushReplacement(
            MaterialPageRoute(builder: (context) => HomePage()));
      } else {
        showDialog(
            // Si non on fait apparaitre une fenetre qui dit que le comtpe n'a pas été verif
            context: context,
            builder: (BuildContext context) {
              return AlertDialog(
                  title: Text("Vérification mail"),
                  content: Text(
                      'Vous devez verifier votre mail avant de pouvoir vous connecter.'),
                  actions: [
                    TextButton(
                      child: Text("Ok"),
                      onPressed: () {
      ...
      // Si probleme rencontre lors de la connexion(pb de mdp, etc...) on fait apparaitre une fennetre qui signale l'erreur
      String a = '';
      switch (err.code) {
        case 'user-disabled':
          {
  ...
  TextEditingController mailController =
      TextEditingController(); // On initialise les controlleurs qui vont permettre de recuperer la valeur saisie dans les TextFormField et la cle qui permet de verif la validite du form
                  ...
                  children: <Widget>[
                    ...
                    TextFormField(
                        ...
                        decoration: InputDecoration(
                            ...
                            hintStyle: TextStyle(
                                color: Colors
                                    .white70)), // Pour avoir le Mail ecrit en fond avant de commencer a ecrire
                        validator: (value) {
...
class CreationCompte extends State<CreerCompte> {
    ...
    await showDialog(
        ...
        builder: (BuildContext context) {
          return AlertDialog(
            ...
            content: Text(
                "En cliquant sur Oui, vous certifiez avoir plus de 16 et accepetez les conditions d'utilisations."),
            actions: [
              TextButton(
                child: Text("Non"),
                onPressed: () {
  ...
  void creationCompte() {
    ...
    FirebaseFirestore firebaseFirestore = FirebaseFirestore
        .instance; // Initialisation de l'instance FireStore qui permet de stocker les donnees
    FirebaseAuth firebaseAuth = FirebaseAuth.instance;
    firebaseAuth
        .createUserWithEmailAndPassword(
            // On cree un moyen d'authentification
      ...
      // On l'emmene sur la page qui dit qu'il doit valider son mail
      Navigator.of(context).pushReplacement(
          MaterialPageRoute(builder: (context) => CompteCree()));
    }).catchError((err) {
      showDialog(
          context:
              context, // Si pb rencontre(deja un compte avec ce mail,etc..) message d'erreur qui s'affiche
          builder: (BuildContext context) {
            return AlertDialog(
              title: Text("Error"),
  ...
  bool val = false;
  bool val2 = false;
  Widget affiche = InkWell(
      child: const Text('J\'accepte les termes et conditions d\'utilisation',
          style: TextStyle(
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
        ...
        child: Scaffold(
            ...
            body: Form(
                key:
                    _formKey, // La cle qui permettra de verifier si le Form est valide
                child: Padding(
                    // Mettre de l'espacement
                          ...
                          if (_key.currentState!.validate()) {
                            ...
                            _firebaseAuth.sendPasswordResetEmail(
                                email: mailRecupController
                                    .text); // Methode de firebase pour envoyer un mail de recuperation de mdp
                            Navigator.of(context).pushReplacement(
                                MaterialPageRoute(
                ...
                child: Column(
                  ...
                  children: <Widget>[
                    ...
                    Padding(
                      padding: const EdgeInsets.fromLTRB(15, 30, 15, 15),
                      child: Text(
                        'Votre compte a bien été crée, veuillez verifier votre email pour valider la création.',
